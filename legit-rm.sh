...
do
    ...
    if ! [ "$check" = "$args" ] ; then
        echo "legit-rm: error: invalid filename '$args'" >&2
        exit 1
    fi
    
    # error if file is directory
    if [ -d "$args" ]; then
        echo "legit-rm: error: '$args' is not a regular file" >&2
        exit 1
    fi
    ...
    if ( ! [ -f "$latestCommit/$args" ] || ! cmp -s "$addFile" "$latestCommit/$args" ); then
        ...
        echo "legit-rm: error: '$args' in repository is different to working file" >&2
        exit 1
    fi
done
